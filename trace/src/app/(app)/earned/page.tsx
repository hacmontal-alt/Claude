...
export default function EarnedPage() {
  const { hasRealData, results, sources, prompts, activeBrand } = useBrand();

  // Earned = prompts where the brand was organically mentioned
  const earnedMentions = useMemo(() => {
    const data = hasRealData ? results : [];
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      {!hasRealData && (
        <div className="mb-4">
          <Callout type="info">
            Showing sample data. Run analysis to see real earned mentions.
          </Callout>
        </div>
      )}
