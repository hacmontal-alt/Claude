...
export default function SourcesPage() {
  const { hasRealData, sources: rawSources, activeBrand, triggerAnalysis } = useBrand();
  ...
  const sources = useMemo(() => {
    ...
    for (const s of rawSources) {
      const existing = domainMap.get(s.domain) || { type: s.content_type ?? "unknown", count: 0, citations: 0, owned: s.is_brand_owned };
      existing.count++;
  ...
  const brandCitations = sources.filter((s) => s.isBrandOwned).reduce((a, s) => a + s.totalCitations, 0);
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      <div className="mb-4">
        {!hasRealData ? (
          ...
          <Callout type="success">
            Showing real cited sources for {activeBrand?.brand_name ?? "your brand"}.
          </Callout>
        )}
      </div>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        <Metric label="Brand Citation Rate" value={`${citationRate}%`} change={!hasRealData ? "+1.8%" : undefined} />
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {sources.map((source) => {
              ...
              return (
                <tr key={source.domain} className="border-b border-[#E8EAEB] hover:bg-[#F8F9FA] transition">
                  <td className="px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${source.isBrandOwned ? "bg-[#EF4623]" : "bg-[#E8EAEB]"}`} />
                      <span className="text-[#2D3B42] font-medium">{source.domain}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <Tag label={source.contentType} color={CONTENT_TYPE_COLOR[source.contentType] ?? "gray"} small />
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    {source.isBrandOwned ? (
                  ...
                  <td className="px-3 py-2.5 text-right">
                    <span className={`text-[11px] font-semibold ${isPositive ? "text-[#047857]" : "text-[#DC2626]"}`}>
                      {trendValue}
                    </span>
                  </td>
                </tr>
